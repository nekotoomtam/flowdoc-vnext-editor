...
mode because only generic `node.delete`, `node.duplicate`, and `node.reorder`
are enabled.
...
- V4 opens with generic block lifecycle operations enabled; text, field-chip,
  live layout, and exact layout remain disabled.
